...
function resetXueNumber(){
	...
	if(confirm('确实要重新开局?')){
		$.getJSON(baseUrl+'/api/diantou/table/AddXue/gameType/2/tableId/'+tableId+'/num_xue/'+num_xue,function(data){
			if(data.code == 1){
...
function cancel(){
	...
	if(confirm('确实要删除'+num_xue+'靴'+num_pu+'铺数据?')){
		$.getJSON(baseUrl+'/api/diantou/table/ClearLuZhu/gameType/2/tableId/'+tableId+'/num_xue/'+num_xue+'/num_pu/'+num_pu,function(data){
			if(data.code == 1){
...
function sentReslt(){
	...
	$.ajax({
         type: "POST",           
         url: baseUrl+'/api/diantou/table/postData/gameType/2',             
         data: postData,                          
